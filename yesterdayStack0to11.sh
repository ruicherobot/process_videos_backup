    ...
    do
        if [[ "$video" == *"$videodate-$timestamp"* ]];then
            ...
            if [[ "$video" == *"cam001"* ]];then
                ...
                ffmpeg -i $video -r 10 -vf "crop=in_w:in_h*6.0/8.0:0:in_h*1.0/8.0,setpts=1/30*PTS,pad=in_w*1.5-5:in_h+10:5:5:white,drawtext=text='$year-$month-$date':x=(w)/1.2-(text_w)/2:y=(h)*1/5:fontsize=50:fontcolor=red,drawtext=text='$hour-$minute to $nexthour-$nextminute':x=(w)/1.2-(text_w)/2:y=(h)*2/5:fontsize=60:fontcolor=yellow,drawtext=text='SCO    2 1':x=(w)/1.2-(text_w)/2:y=(h)*3/5:fontsize=70:fontcolor=blue,drawtext=text='  /       \\\ ':x=(w)/1.2-(text_w)/2:y=(h)*7/10:fontsize=60:fontcolor=black,drawtext=text=' 7 6 5 4 3 ':x=(w)/1.2-(text_w)/2:y=(h)*4/5:fontsize=60:fontcolor=green" /media/data/timelapse/timelapsevideo/timelapse$name.mp4
            fi
        fi
    done
