    ...
    do
        if [[ "$video" == *"$videodate-$timestamp"* ]];then
            ...
            if [[ "$video" == *"cam001"* ]];then
                ffmpeg -i $video -r 10 -vf crop=in_w*2.0/3.0:in_h:in_w*1.0/6.0:0,setpts=1/30*PTS,pad=in_w+10:in_h+10:5:5:purple /media/data/timelapse/timelapsevideo/timelapse$name.mp4
            elif [[ "$video" == *"cam002"* ]];then
                ffmpeg -i $video -r 10 -vf crop=in_w*2.0/3.0:in_h:in_w*1.0/12.0:0,setpts=1/30*PTS,pad=in_w+10:in_h+10:5:5:purple /media/data/timelapse/timelapsevideo/timelapse$name.mp4
            elif [[ "$video" == *"cam003"* ]];then
                ffmpeg -i $video -r 10 -vf crop=in_w*2.0/3.0:in_h:in_w*1.0/3.0:0,setpts=1/30*PTS,pad=in_w+10:in_h+10:5:5:purple /media/data/timelapse/timelapsevideo/timelapse$name.mp4
                ...
                ffmpeg -i $video -r 10 -vf "setpts=1/30*PTS,pad=in_w*2.0:in_h+10:in_w:5:white,drawtext=text='$year-$month-$date':x=(w)/4-(text_w)/2:y=(h)*1/5:fontsize=50:fontcolor=red,drawtext=text='$hour-$minute to $nexthour-$nextminute':x=(w)/4-(text_w)/2:y=(h)*2/5:fontsize=60:fontcolor=yellow,drawtext=text=' 3SCOs   7  ':x=(w)/4-(text_w)/2:y=(h)*3/5:fontsize=60:fontcolor=blue,drawtext=text='6 5 4 3 2 1':x=(w)/4-(text_w)/2:y=(h)*4/5:fontsize=60:fontcolor=green" /media/data/timelapse/timelapsevideo/timelapse$name.mp4
            fi
        fi
    done
