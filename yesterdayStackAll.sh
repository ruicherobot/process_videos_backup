...
do
    ...
    for video in /media/turingalbertsons1/data/$year-$month-$date/*
    do
        if [[ "$video" == *"$videodate-$timestamp"* ]];then
            ...
            if [[ "$video" == *"cam001"* ]];then
                ffmpeg -i $video -r 10 -vf crop=in_w*8.0/9.0:in_h:in_w/9.0:0,setpts=1/30*PTS,pad=in_w+10:in_h+10:5:5:purple /media/turingalbertsons1/data/timelapse/timelapsevideo/timelapse$name.mp4
...
do
    files=""
    for video in /media/turingalbertsons1/data/timelapse/timelapsevideo/*
    do
        if [[ "$video" == *"$videodate-$timestamp"* ]];then
    ...
    " \
    -map "[v]" /media/turingalbertsons1/data/timelapse/$videodate/$itshour/stacktimelapse$videodate$timestamp.mp4
    stlcount=$(($stlcount + 1))
done
...
rm -rf /media/turingalbertsons1/data/timelapse/timelapsevideo
sleep 25
mkdir /media/turingalbertsons1/data/timelapse/timelapsevideo
