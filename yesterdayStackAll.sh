...
do
    ...
    for video in /media/turingalbertsons3/data/$year-$month-$date/*
    do
        if [[ "$video" == *"$videodate-$timestamp"* ]];then
            ...
            if [[ "$video" == *"cam001"* ]];then
                ffmpeg -i $video -r 10 -vf crop=in_w*2.0/3.0:in_h:in_w*1.0/6.0:0,setpts=1/30*PTS,pad=in_w+10:in_h+10:5:5:purple /media/turingalbertsons3/data/timelapse/timelapsevideo/timelapse$name.mp4
            elif [[ "$video" == *"cam002"* ]];then
                ffmpeg -i $video -r 10 -vf crop=in_w*2.0/3.0:in_h:in_w*1.0/12.0:0,setpts=1/30*PTS,pad=in_w+10:in_h+10:5:5:purple /media/turingalbertsons3/data/timelapse/timelapsevideo/timelapse$name.mp4
            elif [[ "$video" == *"cam003"* ]];then
                ffmpeg -i $video -r 10 -vf crop=in_w*2.0/3.0:in_h:in_w*1.0/3.0:0,setpts=1/30*PTS,pad=in_w+10:in_h+10:5:5:purple /media/turingalbertsons3/data/timelapse/timelapsevideo/timelapse$name.mp4
                ...
                ffmpeg -i $video -r 10 -vf "setpts=1/30*PTS,pad=in_w*2.0:in_h+10:in_w:5:white,drawtext=text='$year-$month-$date':x=(w)/4-(text_w)/2:y=(h)*1/5:fontsize=50:fontcolor=red,drawtext=text='$hour-$minute to $nexthour-$nextminute':x=(w)/4-(text_w)/2:y=(h)*2/5:fontsize=60:fontcolor=yellow,drawtext=text=' 3SCOs   7  ':x=(w)/4-(text_w)/2:y=(h)*3/5:fontsize=60:fontcolor=blue,drawtext=text='6 5 4 3 2 1':x=(w)/4-(text_w)/2:y=(h)*4/5:fontsize=60:fontcolor=green" /media/turingalbertsons3/data/timelapse/timelapsevideo/timelapse$name.mp4
            fi
        fi
    done
...
do
    files=""
    for video in /media/turingalbertsons3/data/timelapse/timelapsevideo/*
    do
        if [[ "$video" == *"$videodate-$timestamp"* ]];then
    ...
    " \
    -map "[v]" /media/turingalbertsons3/data/timelapse/$videodate/$itshour/stacktimelapse$videodate$timestamp.mp4
    stlcount=$(($stlcount + 1))
done
...
rm -rf /media/turingalbertsons3/data/timelapse/timelapsevideo
sleep 25
mkdir /media/turingalbertsons3/data/timelapse/timelapsevideo
