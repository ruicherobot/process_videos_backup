...
do
    ...
    for video in /media/turingalbertsons4/data/$year-$month-$date/*
    do
        if [[ "$video" == *"$videodate-$timestamp"* ]];then
            ...
            if [[ "$video" == *"cam001"* ]];then
                ffmpeg -i $video -r 10 -vf crop=in_w*8.0/9.0:in_h:in_w/9.0:0,setpts=1/30*PTS,pad=in_w+10:in_h+10:5:5:purple /media/turingalbertsons4/data/timelapse/timelapsevideo/timelapse$name.mp4
                ...
                ffmpeg -i $video -r 10 -vf "setpts=1/30*PTS,pad=in_w*1.41:in_h+10:0:5:white,drawtext=text='$year-$month-$date':x=(w)*0.85-(text_w)/2:y=(h)*1/5:fontsize=50:fontcolor=red,drawtext=text='$hour-$minute to $nexthour-$nextminute':x=(w)*0.85-(text_w)/2:y=(h)*2/5:fontsize=60:fontcolor=yellow,drawtext=text=' Self Check Out':x=(w)*0.85-(text_w)/2:y=(h)*3/5:fontsize=60:fontcolor=blue,drawtext=text=' 1 2 3 4 5 ':x=(w)*0.85-(text_w)/2:y=(h)*4/5:fontsize=60:fontcolor=green" /media/turingalbertsons4/data/timelapse/timelapsevideo/timelapse$name.mp4
            fi
        fi
    done
...
do
    files=""
    for video in /media/turingalbertsons4/data/timelapse/timelapsevideo/*
    do
        if [[ "$video" == *"$videodate-$timestamp"* ]];then
    ...
    " \
    -map "[v]" /media/turingalbertsons4/data/timelapse/$videodate/$itshour/stacktimelapse$videodate$timestamp.mp4
    stlcount=$(($stlcount + 1))
done
...
rm -rf /media/turingalbertsons4/data/timelapse/timelapsevideo
sleep 25
mkdir /media/turingalbertsons4/data/timelapse/timelapsevideo
